...
namespace ProjectWindowHistory
{
    ...
    public static class ProjectWindowReflectionUtility
    {
        ...
        public static int[] GetLastFolderInstanceIds(EditorWindow targetProjectWindow)
        {
            ...
            // インスタンスID配列にして返す
            var folderInstanceIdsObject = GetFolderInstanceIDsMethod.Invoke(null, new[] { lastFolderPaths });
            return ConvertToInstanceIds(folderInstanceIdsObject);
        }

        private static int[] ConvertToInstanceIds(object value)
        {
            if (value == null)
            {
            ...
            if (value is int[] instanceIds)
            {
                return instanceIds;
            }

            if (value is IEnumerable<int> instanceIdEnumerable)
            {
                return instanceIdEnumerable.ToArray();
            }
            ...
            if (value is IEnumerable enumerable)
            {
                ...
                foreach (var item in enumerable)
                {
                    ...
                    {
                        // Ignore values that cannot be converted to an instance ID.
                    }
                }
